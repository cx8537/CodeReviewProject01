...
std::vector<std::string> FileUtils::getFilesInDirectory(const std::string& path)
{
    ...
    if (!directoryExists(path)) {
        AppLog::error("FileUtils", "Directory does not exist.");
        return files;
    }
...
std::vector<std::string> FileUtils::getFilesInDirectory(const std::string& path, const std::string& extension)
{
    std::vector<std::string> files;
...
bool FileUtils::readBinaryFile(const std::string& path, std::vector<uint8_t>& outData)
{
    if (!fileExists(path)) {
        AppLog::error("FileUtils", "File does not exist.");
        return false;
    }

    std::ifstream file(path, std::ios::binary | std::ios::ate);
    if (!file.is_open()) {
        AppLog::error("FileUtils", "Failed to open file.");
        return false;
    }
